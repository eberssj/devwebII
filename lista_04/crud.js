...
async function deleteArtistas(id_artista) {
    await deleteMusicasByArtist(id_artista);
    await deleteAlbunsByArtist(id_artista);
    const conn = await connect();
...
async function deleteAlbuns(id_album) {
    await deleteMusicasByAlbum(id_album);
    const conn = await connect();
...
async function selectArtistaPorId(id) {  // Função adicionada
    const conn = await connect();
    const [rows] = await conn.query('SELECT * FROM artistas WHERE id_artista = ?', [id]);
    if (rows.length > 0) {
        return rows[0];
    }
    return null;
...
async function deleteMusicasByArtist(id_artista) {
    const conn = await connect();
    const sql = 'DELETE FROM musicas WHERE id_artista = ?;';
    return await conn.query(sql, [id_artista]);
}

async function deleteMusicasByAlbum(id_album) {
    const conn = await connect();
    const sql = 'DELETE FROM musicas WHERE id_album = ?;';
    return await conn.query(sql, [id_album]);
}

async function deleteAlbunsByArtist(id_artista) {
    const conn = await connect();
    const sql = 'DELETE FROM albuns WHERE id_artista = ?;';
    return await conn.query(sql, [id_artista]);
}

async function selectAlbumPorId(id) {
    const conn = await connect();
    const [rows] = await conn.query('SELECT * FROM albuns WHERE id_album = ?', [id]);
    if (rows.length > 0) {
        return rows[0];
    }
    return null;
...
module.exports = {
    ...
    selectAlbumPorId,
    updateMusica,
    deleteMusicasByArtist,
    deleteMusicasByAlbum,
    deleteAlbunsByArtist
};
